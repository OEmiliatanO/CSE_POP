...
\
static void _insert_##__TYPE__(node_##__TYPE__ * now, __TYPE__ val)\
{\
	if (!now) return;\
	if (now->nex == NULL)\
	{\
...
\
static void _remove_##__TYPE__(node_##__TYPE__ * now)\
{\
	if (!now) return;\
...
{\
	...
	{\
		++li->size;\
		_insert_##__TYPE__(li->tail->prev, val);\
		return;\
	}\
	\
	++li->size;\
...
{\
	...
	{\
		--li->size;\
		li->head = li->head->nex;\
		_remove_##__TYPE__(li->head->prev);\
		return;\
	}\
	if (pos == li->size - 1)\
	{\
		--li->size;\
		_remove_##__TYPE__(li->tail->prev);\
		return;\
	}\
	\
	--li->size;\
...
define_list_method(char)
define_list_method(short)
define_list_method(int)
define_list_method(long)
define_list_method(float)
define_list_method(double)

define_list_method(ptr_to_char)
define_list_method(ptr_to_short)
define_list_method(ptr_to_int)
define_list_method(ptr_to_long)
define_list_method(ptr_to_float)
define_list_method(ptr_to_double)
